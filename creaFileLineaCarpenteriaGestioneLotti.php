...
<?php
    ...
    $nome=$_REQUEST["nome"];
    $produzionePerCabina=$_REQUEST["produzionePerCabina"];
    $assemblyID=$_REQUEST["assemblyID"];

    if($produzionePerCabina=="true")
        $colonnaAssemblyID="dbo.distinta_ordini_di_produzione.numero_cabina";
    else
        $colonnaAssemblyID="'$assemblyID'";
    ...
    Material, Thickness, AutoTooling, ScriptTooling, ScriptName, ManualNesting, Drawing, Turret, ProductionLabel, Revision, BendingMode, BendingParameters, StaticNestID, Parameter0, Parameter1, Parameter2, Parameter3, 
    Parameter4, Parameter5, Parameter6, Parameter7, Parameter8, Note
FROM            (SELECT        TOP (100) PERCENT dbo.ordini_di_produzione.nome AS OrderID, dbo.pannelli.codice_pannello AS PartName,
                                   ...
                                   (SELECT        valore
                                     ...
                                     WHERE        (nome = 'Machine')) AS Machine, $colonnaAssemblyID AS AssemblyID, dbo.ordini_di_produzione.data_inizio_produzione AS DueDate, dbo.ordini_di_produzione.finestra AS DateWindow,
                                   (SELECT        valore
                                   ...
                                   (SELECT        valore
                                     ...
                                     WHERE        (nome = 'ManualNesting')) AS ManualNesting, { fn CONCAT({ fn CONCAT
                                   ((SELECT        valore
                                       FROM            dbo.parametri
                                       WHERE        (nome = 'percorso_dxf')), dbo.pannelli_madre.CODSVI) }, '.dxf') } AS Drawing,
                                   (SELECT        valore
                                   ...
                                   (SELECT        valore
                                     ...
                                     WHERE        (nome = 'BendingMode')) AS BendingMode, { fn CONCAT({ fn CONCAT('BaseProgName=', dbo.pannelli_madre.pannello_madre) }, ';') } AS BendingParameters,
                                   (SELECT        valore
...
FROM            (SELECT        TOP (100) PERCENT dbo.ordini_di_produzione.nome AS OrderID, dbo.distinta_ordini_di_produzione.id_distinta AS PartName,
                                                        ...
                                                        (SELECT        valore
                                                          FROM            dbo.parametri_linea_carpenteria AS parametri_linea_carpenteria_19
                                                          WHERE        (nome = 'Machine')) AS Machine, $colonnaAssemblyID AS AssemblyID, dbo.ordini_di_produzione.data_inizio_produzione AS DueDate, 
