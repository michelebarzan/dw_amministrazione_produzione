...
<?php
    ...
    $qPreferiti="SELECT dw_produzione.dbo.pagine_preferite_utenti.id_pagina_preferita_utente, dw_produzione.dbo.pagine_preferite_utenti.utente, dw_produzione.dbo.elenco_pagine.id_pagina, dw_produzione.dbo.elenco_pagine.pagina, dw_produzione.dbo.elenco_pagine.nomePagina, dw_produzione.dbo.elenco_pagine.applicazione, dw_produzione.dbo.elenco_pagine.icona
                FROM dw_produzione.dbo.pagine_preferite_utenti INNER JOIN dw_produzione.dbo.elenco_pagine ON dw_produzione.dbo.pagine_preferite_utenti.pagina = dw_produzione.dbo.elenco_pagine.id_pagina
                WHERE (dw_produzione.dbo.pagine_preferite_utenti.utente = ".$_SESSION['id_utente'].") AND (dw_produzione.dbo.elenco_pagine.applicazione = (SELECT id_applicazione FROM applicazioni where [nomeApplicazione]='dw_amministrazione_produzione'))";
    ...
    $qSezioni="SELECT [id_sezione],[sezione],[descrizione] FROM [dw_produzione].[dbo].[elenco_sezioni] ORDER BY id_sezione";
            ...
            $qPagine="SELECT dbo.elenco_pagine.id_pagina, dbo.elenco_pagine.pagina, dbo.elenco_pagine.nomePagina, dbo.elenco_pagine.applicazione, dbo.elenco_pagine.sezione, dbo.elenco_pagine.icona, dbo.elenco_pagine.descrizione, 
                    ...
                    WHERE (dbo.elenco_pagine.sezione = $id_sezione) AND (dbo.elenco_pagine.id_pagina NOT IN
                            (SELECT elenco_pagine_1.id_pagina
                                ...
                                WHERE (dbo.pagine_preferite_utenti.utente = ".$_SESSION['id_utente'].") AND (elenco_pagine_1.applicazione = (SELECT id_applicazione FROM applicazioni where [nomeApplicazione]='dw_amministrazione_produzione')))) AND (dbo.applicazioni.nomeApplicazione = 'dw_amministrazione_produzione')";
